...
func _physics_process(delta: float) -> void:
	...
	#_segments[_selected_segment].global_position = projected_global_position
	...
	if _segments[_selected_segment].global_position.distance_to(projected_global_position) >= 0.25:
		var distance_ratio: float = clamp(
			_segments[_selected_segment].global_position.distance_to(projected_global_position) / 2.0, 
			0.0, 1.0
		)
...
func _update_segment_positions() -> void:
	for _i: int in range(iterations):
		...
		for i: int in range(_selected_segment + 1, _segments.size()):
			var prev: CharacterBody3D = _segments[i - 1]
			var curr: CharacterBody3D = _segments[i]
			
			var dir: Vector3 = (curr.global_position - prev.global_position).normalized()
			curr.global_position = prev.global_position + dir * _segment_length
		...
		for i: int in range(_selected_segment - 1, -1, -1):
			...
			var dir: Vector3 = (curr.global_position - next.global_position).normalized()
			curr.global_position = next.global_position + dir * _segment_length
...
func _update_segment_rotations() -> void:
	for i: int in range(_segments.size() - 1):
		...
		curr.look_at(next.global_position, Vector3.UP)
